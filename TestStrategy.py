...
class TestStrategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # MACD Strategy
        dataframe = self.calculateMACD(dataframe)

        print("=======================")
        print((dataframe.loc[len(dataframe)-29:,['trend','trend_period','trend_strength','predicted_value','close']]))
    ...
    # Helper Function
    def calculateMACD(self,dataframe):
        # Calculate MACD
        short_window = 12
        long_window = 26
        signal_window = 9

        dataframe['short_ema'] = dataframe['close'].ewm(span=short_window, adjust=False).mean()
        dataframe['long_ema'] = dataframe['close'].ewm(span=long_window, adjust=False).mean()
        dataframe['macd'] = dataframe['short_ema'] - dataframe['long_ema']
        dataframe['signal_line'] = dataframe['macd'].ewm(span=signal_window, adjust=False).mean()
        # Generate buy and sell signals
        dataframe['position'] = 0
        dataframe.loc[(dataframe['macd'] > dataframe['signal_line']), 'position'] = 1  # Buy signal
        dataframe.loc[(dataframe['macd'] < dataframe['signal_line']), 'position'] = -1  # Sell signal
    ...
    def getPredictedValueFromKNN(self,x,dataframe):
        ...
        # 0,size
        for i in range(x.name,max_predict_candle,-1):
